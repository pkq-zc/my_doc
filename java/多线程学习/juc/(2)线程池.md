...
上面已经介绍如何创建线程池,那么线程池创建好了如何提交任务呢?提交任务主要有下面这几种方式:
...
public class App4 {
    public static void main(String[] args) {
        ThreadPoolExecutor executor = new ThreadPoolExecutor(1, 8, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
        //创建10个执行任务并提交
        for (int i = 0; i < 10; i++) {
            CalTask task = new CalTask(i, 1 + i);
...
class CalTask implements Runnable {
    ...
    public CalTask(Integer x, Integer y) {
        this.x = x;
        this.y = y;
    }
    ...
    public void run() {
        ...
        Integer sum = x + y;
        System.out.println(this.toString() + ": sum = " + sum);
    }
...
```java
CalTask{x=0, y=1}: sum = 1
CalTask{x=1, y=2}: sum = 3
CalTask{x=2, y=3}: sum = 5
CalTask{x=3, y=4}: sum = 7
CalTask{x=4, y=5}: sum = 9
CalTask{x=5, y=6}: sum = 11
CalTask{x=6, y=7}: sum = 13
CalTask{x=7, y=8}: sum = 15
CalTask{x=8, y=9}: sum = 17
CalTask{x=9, y=10}: sum = 19
```

任务通过```execute()```提交到线程池之后,再调用```shutdown```等待所有线程执行完成之后关闭线程池.需要注意的是,停止线程池有两种方式:```shutdown```和```shutdownNow```.当执行```shutdown```或者```shutdownNow```,这个时候线程池不再会接受新的任务.这两种方式最大的区别在于停止的方式.调用```shutdown```线程池会等待所有任务执行完成.而```shutdownNow```则会尽力(并不会100%保证)调用线程的```interrupt```来中断正在执行的线程.如果当前任务没有对中断信号做响应,那么任务会继续执行.而那些存在队列中还未执行的任务将不会被执行.修改```shutdown```为```shutdownNow```,打印结果如下:

```java
java.lang.InterruptedException: sleep interrupted
    at java.lang.Thread.sleep(Native Method)
    at com.buydeem.CalTask.run(App4.java:43)
...
可以看见正在执行中的线程被中断了,结果没有别计算出来.而那些还在队列中的任务也没有被执行.
...
从API可以看出,```submit```这种方式提交任务都是有返回值的,返回值的类型为```Future```.同时还有一种我们之前没遇见的参数类型```Callable```.

#### Future && Callable

```Future```是一个接口,它用来代表一个执行结果.那么什么是执行结果呢?就是你提交任务之后,你无法及时的获取到结果.但是在你提交任务完了之后,它会给你一个凭证,你通过这个凭证可以知道任务有没有完成,如果完成了你可以从这个凭证中获取到任务执行完成之后的结果.  
例如去你商城吃饭,可能因为人比较多需要排队.你首先去前台通过关注公众号排队(提交任务),然后你会获取到你的排队编码(异步结果).然后你就可以去商城别的地方逛逛,而不需要在饭店门口坐着等待.你再逛的过程中可以通过手机查看是否已经排到了自己,也可以不管继续闲逛.当到排到你的时候饭店会通过微信号通知告诉你可以吃饭了.你去前台通过之前的排队编码确认就可以开始吃饭了.  

```Callable```同样也是一个接口,它表示返回结果并且可能抛出异常的任务.我们之前通过```Runnable```实现的任务是没有返回值的,通俗的说就是没有返回结果.但是```Callable```类型的任务是有返回值的,是可以返回任务执行完成之后的结果.
...
public class App1 {
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ThreadPoolExecutor executor = new ThreadPoolExecutor(1, 8, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
        //创建任务
        CalTask calTask = new CalTask(1, 2);
        //提交任务
        Future<Integer> future = executor.submit(calTask);
        //future.get()会阻塞等待任务完成
        System.out.println("计算完成,结果为:"+future.get());
...
我们之前的实现```Runnable```接口的任务结果没有办法返回,现在改为实现```Callable```接口的任务,它可以在计算完成之后通过```Future```获取到计算完成的结果.相比于```Runnable```,使用```Callable```这种方式相对来说更加灵活.同样我们```submit```也支持提交```Runnable```类型的任务,只不过它通过```Futrue```获取到的结果为```null```(```submit(Runnable task)```)或者我们在提交任务前设定的值(```submit(Runnable task, T result)```).  
